...
button_conf = ctk.CTkButton(left_frame, image=image_chave, text="", command=open_new_window, width=10, height=40, fg_color='#585858', text_color='black', hover_color='#7E81BD', corner_radius=8)
button_conf.pack(pady=25, padx=10)

button_iperf = ctk.CTkButton(left_frame, image=image_iperf, text="", command=iperf, width=10, height=40, fg_color='#585858', text_color='black', hover_color='#7E81BD', corner_radius=8)
button_iperf.pack(pady=25, padx=10)

button_iperf = ctk.CTkButton(left_frame, image=image_iperf_plug, text="", command=iperf_plug, width=0, height=0, fg_color='#585858', text_color='black', hover_color='#7E81BD', corner_radius=8)
button_iperf.pack(pady=25, padx=10)

button_ip = ctk.CTkButton(left_frame, image=image_ips, text="", command=openipview, width=10, height=40, fg_color='#585858', text_color='black', hover_color='#7E81BD', corner_radius=8)
...
def disable(i):
    try:
        ...
        subprocess.run(["/bin/bash", "/home/tibo/Documentos/LinuxNetMaster/scripts/disable_interfaces.sh", str(i)], check=True)
...
def reset(i):
    try:
        ...
        subprocess.run(["/bin/bash", "/home/tibo/Documentos/LinuxNetMaster/scripts/reset_all.sh", str(i)], check=True)
        print("Script executado com sucesso!", i)
    except subprocess.CalledProcessError as e:
        print(f"Erro ao executar o script: {e}")
